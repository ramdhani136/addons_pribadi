...
# Copyright (c) 2023, Ilham Ramdhani and contributors
# Untuk informasi lisensi, silakan lihat license.txt
    ...
    def validate(self):
        if self.valid_from_date and self.valid_to_date:
            if self.valid_from_date > self.valid_to_date:
                frappe.throw("Tanggal tidak valid, harap periksa kembali.")

        if self.filter_by == "Item Code":
            if not self.item_code_table:
                frappe.throw("Silakan atur Kode Barang Anda.")
            else:
            ...
            if not self.parent_item_group:
                frappe.throw("Silakan atur Kelompok Barang Anda.")
        ...
        # Mengasumsikan update_min_price_items() didefinisikan di tempat lain
